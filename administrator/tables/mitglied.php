...
/**
 * mitglied Table class
 */
class FirefightersTablemitglied extends JTable
{
	...
	{
		parent::__construct('#__firefighters', 'id', $db);
	}
 ...
 if((!empty($array['list_dienstgrad'])&&(is_array($array['list_dienstgrad'])))) { $this->_jsonEncode[]="list_dienstgrad"; }
			...
			if(!empty($array['dienstgrad'])){
				...
				else if(strrpos($array['dienstgrad'], ',') != false){
					$array['dienstgrad'] = explode(',',$array['dienstgrad']);
				}
			}
			else {
				$array['dienstgrad'] = '';
 ...
 if((!empty($array['list_abteilungen'])&&(is_array($array['list_abteilungen'])))) { $this->_jsonEncode[]="list_abteilungen"; }
			...
			if(!empty($array['abteilungen'])){
				...
				else if(strrpos($array['abteilungen'], ',') != false){
					$array['abteilungen'] = explode(',',$array['abteilungen']);
				}
			}
			else {
				$array['abteilungen'] = '';
 ...
 if((!empty($array['list_ausbildungen'])&&(is_array($array['list_ausbildungen'])))) { $this->_jsonEncode[]="list_ausbildungen"; }
		...
		if(!empty($array['ausbildungen'])){
				...
				else if(strrpos($array['ausbildungen'], ',') != false){
					$array['ausbildungen'] = explode(',',$array['ausbildungen']);
				}
			}
			else {
				$array['ausbildungen'] = '';
		...
		//Support for multiple or not foreign key field: missions_eiko
			if(!empty($array['missions_eiko'])){
				if(is_array($array['missions_eiko'])){
					$array['missions_eiko'] = implode(',',$array['missions_eiko']);
				}
				else if(strrpos($array['missions_eiko'], ',') != false){
					$array['missions_eiko'] = explode(',',$array['missions_eiko']);
				}
			}
			else {
				$array['missions_eiko'] = '';
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_firefighters.mitglied.'.$array['id']) && $array['state'] == 1)){
			$array['state'] = 0;
		}
		if($array['id'] == 0){
	...
	protected function _getAssetName()
	{
		...
		return 'com_firefighters.mitglied.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_firefighters');
